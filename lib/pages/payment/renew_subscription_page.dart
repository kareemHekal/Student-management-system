...
class _SubscriptionPlansPageState extends State<SubscriptionPlansPage>
  ...
  // ---------------------------------------------------------------------------
  // منطق الدفع (Logic) المربوط بالـ Provider
  // ---------------------------------------------------------------------------

  Future<void> _handlePaymentSequence(Subscription plan) async {
    final teacher = context.read<TeacherProvider>().teacher;
    if (teacher == null) {
      AppSnackBars.showError(context, "عفواً، لم يتم العثور على بيانات المدرس");
      return;
    }

    // توليد ID للفاتورة القادمة
    final String futureBillId = FirebaseFirestore.instance
        .collection('teachers')
        .doc(teacher.id)
        .collection('bills')
        .doc()
    ...
    final String customRef =
        "STUDENIZER_${teacher.id}_${DateTime.now().millisecondsSinceEpoch}";

    String? paymentUrl;

    // --- التغيير الجذري هنا: الحفظ في Firestore بدل SharedPreferences ---
    // بنعمل Document مؤقت اسمه pending_payments جوه المدرس
    final pendingData = {
      'ref': customRef,
      'billId': futureBillId,
      'plan': plan.toJson(), // الفايربيز بيقبل Map عادي مش لازم Json String
    ...
    await runWithLoading(
      context,
      () async {
        // 1. تسجيل نية الدفع في السحابة
        await FirebaseFirestore.instance
            .collection('teachers')
            .doc(teacher.id)
            .collection('pending_payments')
            .doc(customRef) // بنستخدم الـ Ref كـ ID للوثيقة عشان سهولة البحث
            .set(pendingData);

        // 2. طلب الرابط من EasyKash
        paymentUrl = await EasyKashService.createPaymentLink(
          amount: plan.price.toDouble(),
          orderId: futureBillId,
          customerName: teacher.name,
          customerEmail: teacher.email,
          customerPhone: teacher.phoneNumber,
          reference: customRef,
          redirectUrl: studenizerRedirect,
        );

        if (paymentUrl == null) throw Exception("فشل في الحصول على رابط الدفع");
      },
      loadingText: "جاري تجهيز بوابة الدفع...",
    );

    if (paymentUrl != null && mounted) {
      // 3. فتح الـ WebView
      final bool? isReturn = await Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => EasyKashWebViewPage(
      ...
      // 4. التحقق عند العودة الفورية
      if (isReturn == true && mounted) {
        // هنا بنبعت الـ teacher.id عشان نعرف نمسح الداتا منين
        bool isDone = await _verifyAndActivateSubscription(
            customRef, plan, futureBillId, teacher.id);

        if (isDone) {
          _showSuccessDialog();
          // الحذف بيتم جوه دالة الـ verify لو نجحت
        }
      }
    }
  }

  Future<bool> _verifyAndActivateSubscription(
      String ref, Subscription plan, String billId, String teacherId) async {
    ...
    await runWithLoading(
      context,
      () async {
        statusData = await EasyKashService.checkPaymentStatus(ref);
        ...
        if (statusData != null &&
          ...
          if (isBoost) {
            await FirebaseFunctions.renewBoostSubscription(
          ...
          // تحديث الـ Provider
          if (mounted) {
            await context.read<TeacherProvider>().refreshTeacherData();
          }

          // --- مسح عملية الدفع المعلقة من الفايربيز لأنها تمت ---
          await FirebaseFirestore.instance
              .collection('teachers')
              .doc(teacherId)
              .collection('pending_payments')
          ...
          String msg = statusData?['status'] == "PENDING"
              ? "العملية لا تزال قيد المعالجة."
              : "فشلت عملية الدفع.";
          throw Exception(msg);
        }
      },
          ...
          children: [
            Row(
              ...
              children: [
                ...
                _buildPriceTag(plan.price.toString()),
              ],
            ),
            ...
            Text(plan.description,
                style: TextStyle(
                    color: Colors.blueGrey[400], fontSize: 13, height: 1.4)),
            const Padding(
            ...
            Row(
              ...
              children: [
                ...
                _buildFeatureIcon(
                    Icons.calendar_today_rounded, "${plan.durationInDays} يوم"),
              ],
            ),
  ...
  Widget _buildSubscribeButton(Subscription plan) {
    return SizedBox(
      width: double.infinity,
      height: 55,
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "أنت على وشك الاشتراك في ${plan.name} بمبلغ ${plan.price} ج.م",
            ...
            GestureDetector(
              ...
              child: Text(
                "بإتمام الاشتراك، أنت توافق على الشروط وسياسة الخصوصية",
                textAlign: TextAlign.center,
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text("تم تفعيل اشتراكك الجديد بنجاح، يمكنك الآن متابعة عملك.",
                textAlign: TextAlign.center),
